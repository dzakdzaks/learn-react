...
import EmployeeEdit from "../model/EmployeeEdit";

function EditEmployee(employee: EmployeeEdit) {
  const [name, setName] = useState(employee.data.name);
  const [role, setRole] = useState(employee.data.role);
      ...
      >
        ...
        <Modal.Body>
          <form
            onSubmit={(e) => {
              e.preventDefault();
              employee.updateEmployee(employee.data.id, name, role);
            }}
            id="editmodal"
